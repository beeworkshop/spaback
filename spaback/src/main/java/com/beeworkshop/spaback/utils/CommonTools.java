...
import com.beeworkshop.spaback.commons.exception.CommonJsonException;

/**
 * 
 * @author beeworkshop
 * @description 实现常用，共用的工具功能
 */
public class CommonTools {
	 ...
	 * 返回错误信息JSON
	 */
	public static JSONObject errorJson(ErrorDef errDef) {
		JSONObject resultJson = new JSONObject();
		resultJson.put("code", errDef.getErrCode());
		resultJson.put("msg", errDef.getErrMsg());
		resultJson.put("info", new JSONObject()); // 统一消息结构
		return resultJson;
	}
	...
	public static void hasAllRequired(final JSONObject jsonObject, String requiredColumns) {
		if (!isNullOrEmpty(requiredColumns)) {
			...
			if (!isNullOrEmpty(missCol)) {
				jsonObject.clear();
				jsonObject.put("code", ErrorDef.E90003.getErrCode());
				jsonObject.put("msg", "缺少必填参数:" + missCol.trim());
				jsonObject.put("info", new JSONObject()); // 统一返回json的格式
